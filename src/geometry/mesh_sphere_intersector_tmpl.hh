  ...
  nb_prim_by_el(0)
{
  ...
  for (ghost_type_t::iterator gt = ghost_type_t::begin();  gt != ghost_type_t::end(); ++gt) {
    ...
    // std::cout << "new_node_per_elem size = " << this->new_node_per_elem(type,ghost_type).getSize() << " (type = " << type << ", ghost_type = " << ghost_type << ")" << std::endl;
  }
...
#if defined(AKANTU_IGFEM)
    new_nodes.setNewNodePerElem(this->new_node_per_elem);
