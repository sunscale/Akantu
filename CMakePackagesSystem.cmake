...
#===============================================================================
function(generate_svn_auth authfile svn_authz_file)
  file(STRINGS ${authfile} _authfile)

  file(WRITE ${svn_authz_file} "
# deying list for akantu
")

  foreach(_line ${_authfile})
    string(REGEX MATCH ".* *= *" _group "${_line}")
    string(REPLACE "${_group}" "" _packages "${_line}")
    string(REGEX REPLACE " *= *" "" _group "${_group}")
    string(REGEX REPLACE " +" ";" _packages "${_packages}")

    set(_u_packages)
    foreach(_pkg ${_packages})
      string(TOUPPER ${_pkg} _u_pkg)
      list(APPEND _u_packages ${_u_pkg})
    endforeach()

    foreach(_pkg ${${_project}_PACKAGE_SYSTEM_PACKAGES_NAMES_LIST_ALL})
      list(FIND _u_packages ${_pkg} _res)
      if(_res EQUAL -1)
	list(APPEND ${_pkg}_DENY_GROUP ${_group})
      endif()
    endforeach()
  endforeach()
  ...
  foreach(_pkg ${${_project}_PACKAGE_SYSTEM_PACKAGES_NAMES_LIST_ALL})
    if(${_pkg}_DENY_GROUP)
      ...
      string(TOLOWER ${_pkg} _l_pkg)
      file(APPEND ${svn_authz_file} "
[${${_project}_TRUNK}/packages/${_l_pkg}.cmake]
")
      foreach(_grp ${${_pkg}_DENY_GROUP})
	file(APPEND ${svn_authz_file} "${_grp}=
")
      endforeach()

      foreach(_file ${${_pkg_name}_FILES})
	file(APPEND ${svn_authz_file} "
[${${_project}_TRUNK}/src/${_file}]
")
	foreach(_grp ${${_pkg}_DENY_GROUP})
	  file(APPEND ${svn_authz_file} "${_grp}=
")
	endforeach()
      ...
      if(${_package_name}_TESTS_FILES)
	foreach(_file ${${_package_name}_TESTS_FILES})
	  file(APPEND ${svn_authz_file} "
[${${_project}_TRUNK}/${_file}]
")
	  foreach(_grp ${${_pkg}_DENY_GROUP})
	    file(APPEND ${svn_authz_file} "${_grp}=
")
	...
	endforeach()
      ...
      if(${_package_name}_DOC)
	foreach(_file ${${_package_name}_DOC})
	  file(APPEND ${svn_authz_file} "
[${${_project}_TRUNK}/doc/${_file}]
")
	  foreach(_grp ${${_pkg}_DENY_GROUP})
	    file(APPEND ${svn_authz_file} "${_grp}=
")
