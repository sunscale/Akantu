pipeline {
  ...
  environment { 
    PYTHONPATH = '${WORKSPACE}/test/ci/script/'
    PATH = '${PATH}:${WORKSPACE}/test/ci/script/'
    BLA_VENDOR = 'OpenBLAS'
...
def failed() {
    sh """
       set +x
       export PYTHONPATH=$PWD/tests/ci/scripts
       ./tests/ci/scripts/hbm -a ${API_TOKEN} -b ${BUILD_TARGET_PHID} failed
       """
}
...
def passed() {
    sh """
       set +x
       export PYTHONPATH=$PWD/tests/ci/scripts
       ./tests/ci/scripts/hbm -a ${API_TOKEN} -b ${BUILD_TARGET_PHID} passed
       """
}
...
def createArtifact(artefact) {
  sh """
     set +x
     export PYTHONPATH=$PWD/tests/ci/scripts
     ./tests/ci/scripts/hbm -a ${API_TOKEN} -b ${BUILD_TARGET_PHID} \
